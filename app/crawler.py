...
class ZennCrawler:
    ...
    def __init__(self):
        ...
        self.logger.info("ZennCrawler initialized")
    ...
    def fetch_articles(self, topic: str, max_articles: int = 10) -> List[Dict]:
        ...
        Returns:
            記事リスト
        """
        self.logger.info(f"Fetching articles for topic: {topic}, max_articles: {max_articles}")
        ...
        try:
            ...
            self.logger.warning(f"Feed fetch failed: {e}")
    ...
    def fetch_articles_from_feed(self, topic: str, period: str = None, max_articles: int = None) -> List[Dict]:
        """
        ...
        self.logger.info(f"Fetching from feed: topic={topic}, period={period}, max_articles={max_articles}")
        ...
        try:
            ...
            for item in items[:max_articles * 2]:  # 余裕を持って取得
                try:
                    ...
                    if article and self._is_within_period(article['published_at'], cutoff_date):
                        articles.append(article)
                        if len(articles) >= max_articles:
                            break
                except Exception as e:
                    self.logger.warning(f"Failed to parse feed item: {e}")
                    continue
            
            self.logger.info(f"Successfully fetched {len(articles)} articles from feed")
            return articles
            
        except Exception as e:
            self.logger.error(f"Failed to fetch from feed: {e}")
            return []
